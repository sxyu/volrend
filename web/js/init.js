var Volrend = {
    ...
    onRuntimeInitialized: function() { $(document).ready(() => {
        ...
        document.addEventListener("mousedown", function(e){
            if (e.target.id === "slider-time") {
                return;
            }
            // Mode 0: rotate, 1: pan, 2: zoom
            let mode = 0; // default to rotate
            if (e.button == 2 || (e.button == 0 && e.ctrlKey)) {
                // Pan (right click OR left click + ctrl)
                mode = 1;
            } else if (e.button == 1 || (e.button == 0 && e.altKey)) {
                // Zoom (middle click OR left click + alt)
                mode = 2;
            }
            Volrend.on_mousedown(e.clientX, e.clientY, mode);
        });
        ...
        document.addEventListener("touchstart", function(e){
            ...
            if (e.touches.length > 0) {
                ...
                // Mode 0: rotate, 1: pan, 2: zoom
                Volrend.on_mousedown(touch_cen_x, touch_cen_y, (e.touches.length > 1) ? 1 : 0);
            }
        });
