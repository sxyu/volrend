...
__global__ void query_kernel(const float* data,
    ...
    query_single_from_root(data, child, q, out, N);
}
...
__global__ void precomp_kernel(float* data,
    ...
    rgba[3] = 1.f - expf(-step_sz * fmaxf(rgba[3], 0.f));
}
}  // namespace
}  // namespace device
...
void N3Tree::load_cuda() {
    ...
    cuda(MemcpyAsync(device.offset, offset.data(), 3 * sizeof(float),
                cudaMemcpyHostToDevice));
    cuda_loaded_ = true;
    // -1 means not preprocessed and suitable for querying
    last_alpha_step_ = -1.f;
}
...
void N3Tree::precompute_step(float step_sz) const {
    if (last_alpha_step_ == step_sz) {
        return;
    }
    last_alpha_step_ = step_sz;
